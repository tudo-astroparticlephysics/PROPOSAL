	...
	public:
		...
		Medium();
                ...
                /*!
                 ...
                 The cuts ecut and vcut to be used must satisfy \f$e_{cut}\geq 0\f$ and \f$0 \leq v_{cut} \leq 1\f$; If both values satisfy these
                 inequalities, the lower from \f$ E \cdot v_{cut} \f$ and \f$ e_{cut}\f$ will be used. If only one value satisfies the inequalities,
                 only that one value is used. If both values are outside these intervals, \f$v_{cut}=1\f$ is assumed.
		*/
		...
		// Memberfunctions
			...
			*/
			...
			// Getter
				...
				int get_numCompontents() const {return numCompontents_;}
				std::vector<double> get_NucCharge() const {return nucCharge_;}
				std::vector<double> get_atomicNum() const {return atomicNum_;}
				std::vector<double> get_atomInMolecule() const {return atomInMolecule_;}
				double get_sumCharge() const {return sumCharge_;}

				double get_ZA() const {return ZA_;}
				...
				double get_d0() const {return d0_;}
				double get_r() const {return r_;}

				std::vector<double> get_logConstant() const {return logConstant_;}
				std::vector<double> get_bPrime() const {return bPrime_;}

				double get_rho() const {return rho_;}
				double get_massDensity() const {return massDensity_;}
				double get_molDensity() const {return molDensity_;}
				std::vector<double> get_M() const {return M_;}
				std::vector<std::string> get_E() const {return E_;}
				std::string get_E(int i) const {return E_[i];}
				std::string get_name() const {return name_;}

				double get_ecut() const {return ecut_;}
				double get_vcut() const {return vcut_;}
				double get_vCut() const {return vCut_;}

				Integral get_integral() const {return integral_;}
				std::vector<double> get_mN() const {return mN_;}
				double get_MM() const {return MM_;}
				double get_sumNucleons() const {return sumNucleons_;}
