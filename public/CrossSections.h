...
class CrossSections: public PhysicsModel{
	...
	protected:
		...
		int component_;		// Set to 0 in Constructor
                ...
                // those c' values are values to turn crossections on of or whatever you want to do. Be careful if you change them to another value than 1
		...
		double ini_;
    		...
    		bool df_;		// Set to false in Constructor


		bool jt_;	// Set to false in Constructor
		bool lpm_;	// Set to false in Constructor
	...
	public:
		...
		/// @brief  Necessary to keep class structure, since it is called from subclasses
		CrossSections();

		/// @brief  creates internal references to p and m, to be called from subclasses
		CrossSections(CrossSections *cros);

		/// @brief  initializes all cross sections, creates internal references to p and m
		...
		// Memberfunctions
                ...
                \f[f(E) =- \frac{1}{ \frac{dE}{dx}\big|_{Ioniz} +\frac{dE}{dx}\big|_{Brems}+\frac{dE}{dx}\big|_{PhotoNuc}+\frac{dE}{dx}\big|_{epair}  }\f]
                */
		...
		double function(double E);
                ...
                \f[ dx = \int_{e_i}^{e_f} - \frac{1}{  \frac{dE}{dx}\big|_{Ioniz} +\frac{dE}{dx}\big|_{Brems}+\frac{dE}{dx}\big|_{PhotoNuc}+\frac{dE}{dx}\big|_{epair}  } dE \f]
                */
		double getdx(double ei, double ef, double dist);
                ...
                \f[\int_{e_i}^{\hat{e}_f} - \frac{1}{  \frac{dE}{dx}\big|_{Ioniz} +\frac{dE}{dx}\big|_{Brems}+\frac{dE}{dx}\big|_{PhotoNuc}+\frac{dE}{dx}\big|_{epair}  } dE = dist\f]
                */
		...
		double getef(double ei, double dist);
                ...
                else: \f[return= \int_{e}^{e_{low}} f(E) dE \f] with \f$ e_{low} \f$ energy below which the particle is lost
                */
		double functionInt(double e);
               ...
               Mpairproduction* get_mpairproduction();
                ...
                int get_component() {return component_;}

                double get_ci() const {return ci_;}
                double get_cb() const {return cb_;}
                double get_cp() const {return cp_;}
                double get_ce() const {return ce_;}
                double get_cd() const {return cd_;}

                double get_bremserror() const {return bremserror;}
                double get_epairerror() const {return epairerror;}
                double get_photoerror() const {return photoerror;}
                double get_ionizerror() const {return ionizerror;}



                PROPOSALParticle* get_particle() const {return particle_;}
                Medium* get_medium() const {return medium_;}
                Integral* get_integral() const {return integral_;}

                double get_ini() const {return ini_;}

                bool get_df() const {return df_;}

                Interpolate* get_interpolateJ() const {return interpolateJ_;}
                Interpolate* get_interpolateJdf() const {return interpolateJdf_;}
                bool get_jt() const {return jt_;}
                bool get_lpm() const {return lpm_;}
