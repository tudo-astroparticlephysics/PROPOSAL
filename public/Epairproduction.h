...
class Epairproduction: public CrossSections{
	...
	protected:
		...
		bool init; 		// set to true in Constructors
		double eLpm;
	...
	public:
		...
		//----------------------------------------------------------------------------------------------------//
                ...
                /*!
                 ...
                 sets the parameters \f$v_{Min}=4 \frac{m_e}{E_p}\f$, \f$v_{Max}= max(-\frac{3}{4}\sqrt{e}\frac{m_p}{E_p}Z^{\frac{1}{3}}, 1-6\Big(\frac{m_p}{E_P}\Big)^2, 1-\frac{m_p}{E_p} )\f$
		...
		void setEnergy(int i);
		 ...
		 //----------------------------------------------------------------------------------------------------//
                ...
                /*!
                  ...
                  \f[ f(r) =return= \alpha^2r_e^2 \frac{2Z}{1,5\pi}(Z+k)\frac{1-v}{v}lpm(r^2,b,s)(F_e+\frac{m_e^2}{m_p^2}F_m)\f]
		 */
		...
		//----------------------------------------------------------------------------------------------------//
                ...
                /*!
                  ...
                  if the Landau Pomeranchuk Migdal effect is considered in the calculation, function is modified by a factor
                  \f[lpm=return=\frac{(1+b)(A+(1+r^2)B)+b(C+(1+r^2)D)+(1-r^2)E}{[(2+r^2)(1+b)+x(3+r^2)]\ln\Big(1+\frac{1}{x}\Big)+\frac{1-r^2-b}{1+x}-(3+r^2)}\f]
		 */
		...
		//----------------------------------------------------------------------------------------------------//
                ...
                /*!
                  ...
                  \f[e_{Pair}=return = \rho N_Z z^2 \Big[ \int_{1-r_{max}}^{aux} f(r)dr + \int^1_{aux} f(r) dr \Big]\f]
                  with \f$ aux=max(1-r_{Max}, ComputerPrecision)\f$ and \f$r_{max} = \sqrt{1-\frac{4m_e}{E_p v}}\Big(1-\frac{6m_p^2}{E_p^2(1-v)}\Big)\f$
                 */
		...
		// Getter
                ...
                EpairContinuous* get_Continuous() { return continuous_; }
                EpairStochastic* get_Stochastic() { return stochastic_; }
