...
Every sector needs a medium and a geometry, while it's priority can be scaled with the hierarchy option.
...
A density correction factor for the medium, spezifying the density for the experiment, can optinally be setted.

| Keyword            | Type   | Default   | Description |
| ------------------ | ------ | --------- | ----------- |
| medium             | String |           | Medium in this seector |
| density_correction | Double | 1         | density correction factor of the medium |
| hierarchy          | Double | 1         | decides, which sector is favored, when they overlap |
...
Multiple sectors can be defined. If these sectors overlap, the decision which sector should be prefered is made with the hierarchy option.
