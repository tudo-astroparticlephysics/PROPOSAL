...
double CrossSectionIntegral::CalculatedNdx(double energy)
{
    ...
    for (size_t i = 0; i < components_.size(); ++i)
    {
        ...
        prob_for_component_[i] = parametrization_->GetMultiplier() * dndx_integral_[i].Integrate(
            limits.vUp,
            limits.vMax,
...
double CrossSectionIntegral::CalculatedNdx(double energy, double rnd)
{
    ...
    for (size_t i = 0; i < components_.size(); ++i)
    {
        ...
        prob_for_component_.at(i) = parametrization_->GetMultiplier() * dndx_integral_[i].IntegrateWithRandomRatio(
            limits.vUp,
            limits.vMax,
            std::bind(&Parametrization::FunctionToDNdxIntegral, parametrization_, energy, std::placeholders::_1),
            4,
