...
UtilityInterpolant::UtilityInterpolant(const Utility& utility,
                                       const UtilityInterpolant& collection)
    : UtilityDecorator(utility),
...
double UtilityInterpolant::GetUpperLimit(double ei, double rnd) {
    return std::min(
        std::max(
            ei + rnd / interpolant_diff_->Interpolate(
...
void UtilityInterpolant::InitInterpolation(const std::string& name,
    ...
    interpolants.push_back(std::make_pair(
        &interpolant_, std::bind(&UtilityInterpolant::BuildInterpolant, this,
                                 std::placeholders::_1, std::ref(utility),
                                 std::ref(integral))));
    interpolants.push_back(std::make_pair(
        &interpolant_diff_, std::bind(&UtilityIntegral::FunctionToIntegral,
    ...
    Helper::InitializeInterpolation(name, builder_container, params,
                                    interpolation_def_);
}
...
double UtilityInterpolantDisplacement::Calculate(double ei,
                                                 double ef,
                                                 double rnd) {
    (void)rnd;
    ...
    return std::max((interpolant_diff_->Interpolate((ei + ef) / 2)) * (ef - ei),
                    0.0);
}
...
// ------------------------------------------------------------------------- //
void UtilityInterpolantDisplacement::InitInterpolation(
...
UtilityInterpolantInteraction::UtilityInterpolantInteraction(
    const Utility& utility,
    InterpolationDef def)
    : UtilityInterpolant(utility, def), big_low_(0), up_(0) {
...
double UtilityInterpolantInteraction::Calculate(double ei,
                                                double ef,
                                                double rnd) {
    (void)rnd;
    (void)ef;
...
double UtilityInterpolantInteraction::BuildInterpolant(double energy,
                                                       UtilityIntegral& utility,
                                                       Integral& integral) {
    if (up_) {
        ...
        return -integral.Integrate(
            energy, interpolation_def_.max_node_energy,
            std::bind(&UtilityIntegral::FunctionToIntegral, &utility,
                      std::placeholders::_1),
            4);
    }
...
// ------------------------------------------------------------------------- //
void UtilityInterpolantInteraction::InitInterpolation(
    const std::string& name,
    UtilityIntegral& utility,
    int number_of_sampling_points) {
    Integral integral(IROMB, IMAXS, IPREC2);
    ...
    double a = std::abs(
        -integral.Integrate(particle_def.low, particle_def.low * 10,
                            std::bind(&UtilityIntegral::FunctionToIntegral,
                                      &utility, std::placeholders::_1),
                            4));
    double b = std::abs(
        -integral.Integrate(interpolation_def_.max_node_energy,
...
double UtilityInterpolantDecay::BuildInterpolant(double energy,
                                                 UtilityIntegral& utility,
                                                 Integral& integral) {
...
double UtilityInterpolantTime::BuildInterpolant(double energy,
                                                UtilityIntegral& utility,
                                                Integral& integral) {
    return integral.Integrate(energy, utility_.GetParticleDef().low,
                              std::bind(&UtilityIntegral::FunctionToIntegral,
...
UtilityInterpolantContRand::UtilityInterpolantContRand(const Utility& utility,
    ...
    InitInterpolation("contrand", utility_contrand,
                      def.nodes_continous_randomization);
}
...
double UtilityInterpolantContRand::BuildInterpolant(double energy,
                                                    UtilityIntegral& utility,
                                                    Integral& integral) {
    return integral.Integrate(energy, utility_.GetParticleDef().low,
                              std::bind(&UtilityIntegral::FunctionToIntegral,
...
// ------------------------------------------------------------------------- //
void UtilityInterpolantContRand::InitInterpolation(
    const std::string& name,
    UtilityIntegral& utility,
    int number_of_sampling_points) {
    UtilityInterpolant::InitInterpolation(name, utility,
...
double UtilityInterpolantScattering::Calculate(double ei,
                                               double ef,
                                               double rnd) {
    (void)rnd;
...
double UtilityInterpolantScattering::BuildInterpolant(double energy,
                                                      UtilityIntegral& utility,
                                                      Integral& integral) {
    return integral.Integrate(energy, interpolation_def_.max_node_energy,
                              std::bind(&UtilityIntegral::FunctionToIntegral,
...
// ------------------------------------------------------------------------- //
void UtilityInterpolantScattering::InitInterpolation(
    const std::string& name,
    UtilityIntegral& utility,
    int number_of_sampling_points) {
    UtilityInterpolant::InitInterpolation(name, utility,
