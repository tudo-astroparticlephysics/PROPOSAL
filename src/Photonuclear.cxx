...
using namespace std;
	...
	Photonuclear::Photonuclear(CrossSections *cros): CrossSections(*cros){
			...
			default:
				...
				if(min>max){
					return 0;
                                ...
                                //return integral_->integrateWithLog(min, max, this);
	...
	void Photonuclear::enableHardBB(){
		if(initH){
			initH=false;
			const string gcj="";  // gcj would not run with 3d arrays without this
	...
	double Photonuclear::function(double Q2){
		...
		case 3:  // Abramowicz, Levin, Levy, and Maor parametrization
			...
			if(t<0){ // gcj 3.x on Linux may create negative values here
				t=0;
			}
	...
	// Getter
        ...
        PhotoContinuous*  Photonuclear::get_Continuous() {return continuous_;}
        PhotoStochastic* Photonuclear::get_Stochastic() {return stochastic_;}

        //Setter

        void Photonuclear::set_jt(bool newjt) { jt_ = newjt;}
        void Photonuclear::set_form(int newForm){ form = newForm; }
        void Photonuclear::set_bb(int newbb){bb = newbb;}
        void Photonuclear::set_shadow(int newshadow){shadow =newshadow;}
