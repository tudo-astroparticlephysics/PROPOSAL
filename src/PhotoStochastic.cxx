...
using namespace std;
	...
	PhotoStochastic::PhotoStochastic(Photonuclear *cros):Photonuclear(*cros){
		// ?????? super(cros);
...
/*
cout<<"In PhotoStochastic::dNdx(double rnd): jt_ "<<jt_<<endl;
if(jt_){
for(int i =0; i<(medium_->get_numCompontents()); i++){
cout<<"In PhotoStochastic::dNdx(double rnd): cros->get_cp() = "<<cros->get_cp()<<"interpolateJo_at("<<i<<") = " <<interpolateJo_.at(i)<<"\t interpolateJ_at("<<i<<") = " <<interpolateJ_.at(i)<<endl;
}
}
*/
		if(cros->get_cp()<=0){
