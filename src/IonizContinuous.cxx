...
using namespace std;
	...
	IonizContinuous::IonizContinuous(Ionizationloss *cros) : Ionizationloss(*cros) {
                jt_ = false;
		//??????? super(cros);
...
;
	...
	double IonizContinuous::dEdx(){
		...
		if(cros->get_ci()<=0){
			...
			return max(interpolateJ_->interpolate(particle_->e),(double) 0);
		}
		double result, aux;
...
//cout<<"In IonizConitnuous::DEdx:88"<<vMin<<"\t"<<vUp<<endl;
    ...
    // Setter
        void IonizContinuous::set_jt(bool newJT) { jt_ = newJT;};
