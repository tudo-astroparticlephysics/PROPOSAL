...
using namespace std;
     ...
     * The cuts ecut and vcut to be used must satisfy ecut&gt;0 and 0&lt;=vcut&lt;=1; If both values satisfy these
     * inequalities, the lower from E*vcut and ecut will be used. If only one value satisfies the inequalities,
	...
	if(equalsIgnoreCase(w,"water")) initWater();
	else if(equalsIgnoreCase(w,"ice")) initIce();
	else if(equalsIgnoreCase(w,"salt")) initSalt();
	else if(equalsIgnoreCase(w,"standard rock")) initStandardrock();
	else if(equalsIgnoreCase(w,"frejus rock")) initFrejusrock();
	else if(equalsIgnoreCase(w,"iron")) initIron();
	else if(equalsIgnoreCase(w,"hydrogen")) initHydrogen();
	else if(equalsIgnoreCase(w,"lead")) initLead();
	else if(equalsIgnoreCase(w,"uranium")) initUranium();
	else if(equalsIgnoreCase(w,"air")) initAir();
	else if(equalsIgnoreCase(w,"mineral oil")) initParaffin();
	else if(equalsIgnoreCase(w,"antares water")) initAntaresWater();
	else {
	...
	for(i=0;i<numCompontents_;i++){
	    ...
	    if(nucCharge_.at(i)!=1) flag=true;
	}
	sumCharge_=aux1;
	sumNucleons_=aux2;
	ZA_=aux1/aux2;
	...
	if(flag){
	    ...
	    for(i=0;i<numCompontents_;i++) if(nucCharge_.at(i)!=1){
	...
	E_.at(0)="Na";
	E_.at(1)="Cl";
	nucCharge_.at(0)=11; // H
	nucCharge_.at(1)=17; // O
	atomicNum_.at(0)=22.98977;
	atomicNum_.at(1)=35.4527;
	atomInMolecule_.at(0)=1;
	atomInMolecule_.at(1)=1;
	I_=175.3;  // Calculated by ESTAR detabase (it could be 185 eV by the method of reference below)
	...
	nucCharge_.at(0)=11; // Ionization potential and density corrections
	atomicNum_.at(0)=22; // are close to those of calcium carbonate
	atomInMolecule_.at(0)=1;
	I_=136.4;
	C_=-3.7738;
	a_=0.08301;
     ...
     *==========================================================================
     */
    void Medium::initAntaresWater(){ // added by Claudine Colnard,
	...
	I_ = 75.0;     // All the same as for pure water
	C_ = -3.5017;
	a_ = 0.09116;
	m_ = 3.4773;
	X0_ = 0.2400;
	X1_ = 2.8004;
	d0_ = 0;
	massDensity_ = 1.03975; // J.Brunner, ANTARES-Site/2000-001, the mean value
	...
	aux=ecut_/E;
        if(ecut_>0){
            if(vcut_>0 && vcut_<=1){
                if(aux<vcut_) vCut_=aux;
		else vCut_=vcut_;
	    }
	    else vCut_=aux;
	}
	else{
	    if(vcut_>0 && vcut_<=1) vCut_=vcut_;
	    else vCut_=1.;
	}
