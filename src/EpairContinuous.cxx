...
using namespace std;
        ...
        EpairContinuous::EpairContinuous(Epairproduction *cros) : Epairproduction(*cros)
        {
            reverse_ = false;
            jt_ = false;
        ...
        double EpairContinuous::dEdx()
        {
            ...
            for(i=0; i<medium_->get_numCompontents(); i++)
            {
                ...
                double r1=0.8, rUp=vUp*(1-HALF_PRECISION);
                bool rflag=false;
          //  cout<<"r1 = "<<r1<<"\t "<<"vMin = "<<vMin<<"\t"<<"vUp = "<<vUp<<endl;
                if(r1<rUp)
                {
                    //cout<<2*function(r1)<<"\t"<<function(rUp)<<endl;
