...
class CrossSectionDNDXInterpolant : public CrossSectionDNDXIntegral , public CrossSectionInterpolantBase {
    std::unique_ptr<Interpolant> dndx;
    std::unique_ptr<Interpolant> dndx1d;

    std::unique_ptr<Interpolant> build_dndx(crosssection::Parametrization const&, ParticleDef const&);
    std::unique_ptr<Interpolant> build_dndx1d(crosssection::Parametrization const&, ParticleDef const&);
public:
    template <typename Param, typename Particle, typename Target>
    CrossSectionDNDXInterpolant(const Param& _param, const Particle& _p_def, const Target& _target, std::shared_ptr<const EnergyCutSettings> _cut)
