...
def create_tables(dir_name, interpolate=False, **kwargs):
    ...
    for key in kwargs:
        if key == "dNdx" and kwargs[key] is True:
            f_dNdx = open(dir_name + "Anni_dNdx{}.txt".format("_interpol" if interpolate else ""), "a")
            buf["dNdx"] = [f_dNdx, [""]]
        if key == "dNdx_rnd" and kwargs[key] is True:
            f_dNdx_rnd = open(dir_name + "Anni_dNdx_rnd{}.txt".format("_interpol" if interpolate else ""), "a")
            buf["dNdx_rnd"] = [f_dNdx_rnd, [""]]
        if key == "stoch" and kwargs[key] is True:
            f_stoch = open(dir_name + "Anni_e{}.txt".format("_interpol" if interpolate else ""), "a")
            buf["stoch"] = [f_stoch, [""]]
